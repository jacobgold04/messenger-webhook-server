...
## Step 3: Securely Expose Local Ollama (Cloudflare Tunnels)
We need a permanent public URL that forwards messages safely into your local PC's Ollama instance.
1. Download **cloudflared** for Windows:
   - Go to the [Cloudflare Tunnel Downloads Page](https://developers.cloudflare.com/cloudflare-one/connections/connect-networks/downloads/) and download the `.msi` or `.exe` for Windows 64-bit.
   - Install it.
2. Open your Windows **Command Prompt** (as Administrator).
3. If you do not have a Cloudflare account or domain, you can run a free, anonymous "Quick Tunnel":
   ```bash
   cloudflared tunnel --url http://127.0.0.1:11434
   ```
4. It will print out a URL that looks like `https://funny-words-here.trycloudflare.com`.
   - **Copy this URL.**
   - *Note: Leave this Command Prompt window open whenever you want the bot to be able to "think". If you close it, the tunnel stops.*
5. Go back to your **Render Web Service -> Environment**. Add a new variable:
   - Key: `OLLAMA_PROXY_URL`
   - Value: `https://funny-words-here.trycloudflare.com` *(Do NOT add `/api/chat` to the end, just the base URL!)*
